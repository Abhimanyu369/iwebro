...
const Add_team2 = () => {
  ...
  const fetchData = async (data)=>{
    ...
    let fdata = await axios.post(BACKEND_COMMAN_URL+'/api/addTeam2',formData, headers);
    if(fdata.status === 200){
      navigate('/View_team2');
    }
    else{
      ...
      <div className="container">
        <form onSubmit={formSubmit} method="post" encType="multipart/form-data" className='flex flex-wrap'>
          <div className="w-full">
            <h2 className="mb-4" style={{ letterSpacing: '3px', fontWeight: 600 }}>Add Team 2 </h2>
