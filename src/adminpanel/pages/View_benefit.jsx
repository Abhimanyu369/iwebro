...
const View_benefit = () => {
  ...
  const Delete = async (id) => {
    let data = await axios.delete(`${BACKEND_COMMAN_URL}/api/deleteBenefit/${id}`, headers);
    if (data.status === 200) {
      fetchData();
    }
    else {
    ...
    <><Header />
      ...
      <div className="ml-305 px-4 md:px-6 2xl:px-10">
        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mt-3">
          <h2 className="text-title-md2 font-bold text-black"> Benefits </h2>
          <Link to="/Add_benefit" className="inline-flex items-center justify-center bg-primary py-2 px-10 text-center font-medium text-white hover:bg-opacity-90 lg:px-8 xl:px-10 cursor-pointer">Add Benefit</Link>
        </div>
        {loading ?
                    ...
                    return (
                      <tr key={v._id}>
                        ...
                        <td className='border-b border-[#eee] py-4 px-3 dark:border-strokedark'><div className="flex items-center">
                          <Link to={"/Update_benefit?id=" + v._id}><svg className="feather feather-edit" fill="#fff" height="24" stroke="#0d6efd" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewBox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" /><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" /></svg></Link>
