...
const Update_team2 = () => {
  ...
  const fetchData = async (e)=>{
    ...
    let Fdata = await axios.put(BACKEND_COMMAN_URL+'/api/editTeam2/'+id,formData, headers);
    if(Fdata.status === 200){
      navigate('/View_team2');
    }
    else{
  ...
  const fetchData2 = async ()=>{
    const data = await axios.get(`${BACKEND_COMMAN_URL}/api/singleTeam2/${id}`, headers);
    if(data.status === 200){
  ...
  return (
    ...
    <form onSubmit={formSubmit} method="post" encType="multipart/form-data" className="flex flex-wrap">
     <div className="w-full">
      <h2 className="mb-4" style={{letterSpacing: '3px', fontWeight: 600}}>Edit Team 2 </h2>
