...
const Update_project = () => {
  ...
  const fetchData = async (e) => {
    ...
    let Fdata = await axios.put(BACKEND_COMMAN_URL + '/api/editProject/' + id, formData, headers);
    if (Fdata.status === 200) {
  ...
  const fetchData2 = async () => {
    const data = await axios.get(`${BACKEND_COMMAN_URL}/api/singleProject/${id}`, headers);
    if (data.status === 200) {
        ...
        <div className="container">
          <form onSubmit={formSubmit} method="post" encType="multipart/form-data" className="flex flex-wrap">
            <div className="w-full">
              ...
              <h2 className="mb-4" style={{ letterSpacing: '3px', fontWeight: 600 }}>Edit Project </h2>
