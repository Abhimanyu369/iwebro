...
const View_remote_hiring = () => {
  ...
  const Delete = async (id) => {
    let data = await axios.delete(
      `${BACKEND_COMMAN_URL}/api/deleteRemoteHiring/${id}`,
      headers
    );
    if (data.status === 200) {
      fetchData();
                      ...
                      <tr key={v._id}>
                        ...
                        <td className="border-b border-[#eee] py-4 px-3 dark:border-strokedark">
                          <div className="flex items-center">
                            <Link to={'/Update_remote_hiring?id=' + v._id}>
                              <svg
...
export default View_remote_hiring;
