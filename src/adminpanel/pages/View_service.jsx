...
const View_service = () => {
  ...
  const fetchData = async () => {
    try {
      let data = await axios.get(
        BACKEND_COMMAN_URL + '/api/view_Service',
        headers
      );
      if (data.status === 200) {
        setLoading(true);
  ...
  const Delete = async (id) => {
    let data = await axios.delete(
      `${BACKEND_COMMAN_URL}/api/deleteService/${id}`,
      headers
    );
    if (data.status === 200) {
      fetchData();
    ...
    <>
      ...
      <div className="ml-305 px-4 md:px-6 2xl:px-10">
        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mt-3">
          <h2 className="text-title-md2 font-bold text-black"> Services </h2>
          <Link
            to="/Add_service"
                      ...
                      <tr key={v._id}>
                        ...
                        <td className="border-b border-[#eee] py-4 px-3 dark:border-strokedark">
                          <div className="flex items-center">
                            <Link to={'/Update_service?id=' + v._id}>
                              <svg
