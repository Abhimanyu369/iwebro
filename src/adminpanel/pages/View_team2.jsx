...
const View_team2 = () => {
  ...
  const fetchData = async () => {
    try {
      let data = await axios.get(
        BACKEND_COMMAN_URL + '/api/view_team2',
        headers
      );
      if (data.status === 200) {
        setLoading(true);
  ...
  const Delete = async (id) => {
    let data = await axios.delete(
      `${BACKEND_COMMAN_URL}/api/deleteTeam2/${id}`,
      headers
    );
    if (data.status === 200) {
      fetchData();
    ...
    <>
      ...
      <div className="ml-305 px-4 md:px-6 2xl:px-10">
        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mt-3">
          <h2 className="text-title-md2 font-bold text-black"> Team 2 </h2>
          <Link
            to="/Add_team2"
