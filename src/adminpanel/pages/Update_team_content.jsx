...
const Update_team_content = () => {
  ...
  const fetchData = async (e)=>{
    let Fdata = await axios.put(BACKEND_COMMAN_URL+'/api/editTeamContent/'+id,data, headers);
    if(Fdata.status === 200){
      navigate('/View_team_content');
    }
    else{
      e.preventDefault(); 
    }
  ...
  const fetchData2 = async ()=>{
    const data = await axios.get(`${BACKEND_COMMAN_URL}/api/singleTeamContent/${id}`, headers);
    if(data.status === 200){
        ...
        <div className="container">
          <form onSubmit={formSubmit} method="post" className="flex flex-wrap">
            <div className="w-full">
              <h2 className="mb-4" style={{letterSpacing: '3px', fontWeight: 600}}>Edit Team Content </h2>
...
export default Update_team_content;