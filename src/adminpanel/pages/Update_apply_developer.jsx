...
const Update_apply_developer = () => {
  ...
  const fetchData = async (data) => {
    ...
    let Fdata = await axios.put(BACKEND_COMMAN_URL + '/api/editApplyDeveloper/' + id, formData, headers);
    if (Fdata.status === 200) {
      navigate('/View_apply_developer');
    }
    else {
      navigate('/Update_apply_developer');
    }
  }

  const fetchData2 = async () => {
    const data = await axios.get(`${BACKEND_COMMAN_URL}/api/singleApplyDeveloper/${id}`, headers);
    if (data.status === 200) {
        ...
        <div className="container">
          <form method="post" className="flex flex-wrap" encType="multipart/form-data" onSubmit={formSubmit}>
            <div className="w-full">
              <h2 className="mb-4" style={{ letterSpacing: '3px', fontWeight: 600 }}>Edit Apply Developer </h2>
              {err ? <p style={{ color: 'red' }}>{err}</p> : ''}
              {suc ? <p style={{ color: 'green' }}>{suc}</p> : ''}
            </div>
            {/* <div className="w-full px-3 mb-4">
        <label>Apply Job:-</label>
        <input className="w-full rounded-lg border-[1.5px] border-stroke p-2 px-3 text-dark" type="text" name="jobName" value={data.jobName} onChange={(e)=> setData({...data,jobName : e.target.value})} id='jobName' placeholder="Job Name" />
      </div><br /> */}
...
export default Update_apply_developer;