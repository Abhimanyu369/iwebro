...
const Add_team_content = () => {
  ...
  const fetchData = async ()=>{
    try {
      let fdata = await axios.post(BACKEND_COMMAN_URL+'/api/addTeamContent',data, headers);
      if(fdata.status === 200){
        navigate('/View_team_content');
      }
      else{
        navigate('/Add_team_content');
      }
    } catch (error) {
      console.log(error);
    }
      ...
      <div className="container">
        <form onSubmit={formSubmit} method="post" className='flex flex-wrap'>
          <div className="w-full">
            <h2 className="mb-4" style={{ letterSpacing: '3px', fontWeight: 600 }}>Add Team Content </h2>
...
export default Add_team_content;