...
const View_company = () => {
  ...
  const fetchData = async (page, search, limit) => {
    try {
      ...
      let data = await axios.post(BACKEND_COMMAN_URL + '/api/view_company', credit, headers);
      if (data.status === 200) {
        setLoading(true);
        setOther(data.data);
  ...
  const Delete = async (id) => {
    let data = await axios.delete(`${BACKEND_COMMAN_URL}/api/deleteCompany/${id}`, headers);
    if (data.status === 200) {
      fetchData(page, search, 5);
    }
    else {
    ...
    <><Header />
      <div className="ml-305 px-4 md:px-6 2xl:px-10">
        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mt-3">
          <h2 className="text-title-md2 font-bold text-black"> Talents </h2>
          <input type="text" placeholder="Search Talent" className="rounded-lg border-[1.5px] border-stroke p-2 px-3 text-dark md:w-2/4" onChange={Search} id='search' name='search' />
          <Link to="/Add_talent" className="inline-flex items-center justify-center bg-primary py-2 px-10 text-center font-medium text-white hover:bg-opacity-90 lg:px-8 xl:px-10 cursor-pointer">Add Talent</Link>
        </div>
        {loading ?
          <div>
            <div className="rounded-sm border border-stroke bg-white px-4 pt-6 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 mb-5">
              <div className="max-w-full overflow-x-auto">
                <table className="w-full table-auto" style={{ minWidth: '1800px' }}>
                  <thead>
                    <tr className='text-dark'>
                      <th className='border-b border-[#eee] pb-3 px-3 dark:border-strokedark'>Logo</th>
                      ...
                      <th className='border-b border-[#eee] pb-3 px-3 dark:border-strokedark'>Phone</th>
                      <th className='border-b border-[#eee] pb-3 px-3 dark:border-strokedark'>Address</th>
                      <th className='border-b border-[#eee] pb-3 px-3 dark:border-strokedark'>Location</th>
                      <th className='border-b border-[#eee] pb-3 px-3 dark:border-strokedark'>Description</th>
                      <th className='border-b border-[#eee] pb-3 px-3 dark:border-strokedark'>Pdf</th>
                      <th className='border-b border-[#eee] pb-3 px-3 dark:border-strokedark'>Action</th>
                    </tr>
                  </thead>
                  <tbody>
                    {company.length !== 0 ? company.map((v) => {
                      return (
                        <tr key={v._id}>
                          <td className='border-b border-[#eee] py-4 px-3 dark:border-strokedark'><img src={BACKEND_COMMAN_URL + "/" + v.photo} alt='img' width="100px" /></td>
                          <td className='border-b border-[#eee] py-4 px-3 dark:border-strokedark'>{v.company_name}</td>
                          <td className='border-b border-[#eee] py-4 px-3 dark:border-strokedark'>{v.email}</td>
                          <td className='border-b border-[#eee] py-4 px-3 dark:border-strokedark'>{v.phone}</td>
                          <td className='border-b border-[#eee] py-4 px-3 dark:border-strokedark'>{v.address}</td>
                          <td className='border-b border-[#eee] py-4 px-3 dark:border-strokedark'>{v.location}</td>
                          <td className='border-b border-[#eee] py-4 px-3 dark:border-strokedark'>{v.desc}</td>
                          <td className='border-b border-[#eee] py-4 px-3 dark:border-strokedark'><Link to={BACKEND_COMMAN_URL + "/" + v.pdf} className='text-[#0d6efd]' target='_blank'>Download Pdf</Link></td>
                          <td className='border-b border-[#eee] py-4 px-3 dark:border-strokedark'><div className="flex items-center">
                            <Link to={"/Update_talent?id=" + v._id}><svg className="feather feather-edit" fill="#fff" height="24" stroke="#0d6efd" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewBox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" /><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" /></svg></Link>
            ...
            {other.totalData > 5 ? <div className="p-paginator p-component p-paginator-bottom py-4 flex justify-between items-center mb-20">
              ...
              <div className='sm:mb-0 mb-3'>
                <label className='pe-2 ps-auto'>Talents per page: </label>
