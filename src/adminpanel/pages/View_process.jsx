...
const View_process = () => {
  ...
  const Delete = async (id) => {
    let data = await axios.delete(
      `${BACKEND_COMMAN_URL}/api/deleteProcess/${id}`,
      headers
    );
    if (data.status === 200) {
      fetchData();
  ...
  const formImg = async (e) => {
    const formData = new FormData();
    formData.append('img', e.target.files[0]);
    let data = await axios.post(
      `${BACKEND_COMMAN_URL}/api/addProcessImg`,
      formData,
      headers
    );
    if (data.status === 200) {
    ...
    <>
      ...
      <div className="ml-305 px-4 md:px-6 2xl:px-10">
        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mt-3">
          <h2 className="text-title-md2 font-bold text-black"> Processes </h2>
          <form method="post" encType="multipart/form-data">
            ...
            <input
              ...
              name="photo"
              id="photo"
              className="img mb-4 hidden"
              onChange={formImg}
            />
          </form>
          <Link
            to="/Add_process"
            className="inline-flex items-center justify-center bg-primary py-2 px-10 text-center font-medium text-white hover:bg-opacity-90 lg:px-8 xl:px-10 cursor-pointerm"
          >
            Add Process
          </Link>
        </div>
                      ...
                      <tr key={v._id}>
                        ...
                        <td className="border-b border-[#eee] py-4 px-3 dark:border-strokedark">
                          <div className="flex items-center">
                            <Link to={'/Update_process?id=' + v._id}>
                              <svg
