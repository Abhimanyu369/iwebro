...
const View_job = () => {
  ...
  const fetchData = async (page, search, limit) => {
    try {
      ...
      let data = await axios.post(
        BACKEND_COMMAN_URL + '/api/view_job_admin',
        credit,
        headers
      );
      if (data.status === 200) {
  ...
  const Delete = async (id) => {
    let data = await axios.delete(
      `${BACKEND_COMMAN_URL}/api/deleteJob/${id}`,
      headers
    );
    if (data.status === 200) {
  ...
  const formCheck = async (e, id) => {
    if (e.target.checked) {
      await axios.put(BACKEND_COMMAN_URL + '/api/editJob/' + id, {
        status: true,
      });
      setCheck((v) => {
        return [...v, id];
    ...
    <>
      ...
      <div className="ml-305 px-4 md:px-6 2xl:px-10">
        <div className="mb-12  flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <h2 className="text-title-md2 font-bold text-black mb-0">
            {' '}
            Job Requirements{' '}
          </h2>
          <input
            ...
            className="rounded-lg border-[1.5px] border-stroke p-2 px-3 text-dark md:w-2/4"
            onChange={Search}
            id="search"
            name="search"
          />
          <Link
            to="/Add_job"
            className="inline-flex items-center justify-center bg-primary py-2 px-10 text-center font-medium text-white hover:bg-opacity-90 lg:px-8 xl:px-10 cursor-pointer"
          >
            Add Job Requirement
          </Link>
        </div>
                ...
                >
                  <thead>
                    <tr className="text-dark">
                      <th className="border-b border-[#eee] pb-3 px-3 dark:border-strokedark">
                        Job Name
                      </th>
                      <th className="border-b border-[#eee] pb-3 px-3 dark:border-strokedark">
                        company Name
                  ...
                  <tbody>
                    {job.length !== 0 ? (
                      job.map((v, i) => {
                        return (
                          <tr key={v._id}>
                            <td className="border-b border-[#eee] py-4 px-3 dark:border-strokedark text-capitalize">
                              <Link
                                className="text-[#0d6efd]"
                                to={'/job_profile?id=' + v._id}
                                state={{
                                  company_name: v?.companyId?.company_name,
                                  job_name: v.jobName,
                                }}
                              >
                                {v.jobName}
                            ...
                            <td className="border-b border-[#eee] py-4 px-3 dark:border-strokedark">
                              <label className="relative inline-flex items-center cursor-pointer">
                                <input
                                  ...
                                  onChange={(e) => formCheck(e, v._id)}
                                  checked={check.includes(v._id)}
                                  value={v._id}
                                />
                                <div className="shadow-none w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600"></div>
                              </label>
                            </td>
                            <td className="border-b border-[#eee] py-4 px-3 dark:border-strokedark">
                              <div className="flex items-center">
                                <Link to={'/Update_job?id=' + v._id}>
                                  <svg
                      ...
                      <tr>
                        <td>
                          <h3 className="mb-0 py-4">Data Not Found</h3>
                        </td>
                      </tr>
                    )}
                  </tbody>
            ...
            <div className="p-paginator p-component p-paginator-bottom py-4 flex flex-wrap justify-between items-center mb-20">
              <div className="sm:mb-0 mb-3">
                <button
                  ...
                  className={
                    addClass
                      ? 'disabled p-paginator-prev p-paginator-element p-link me-2'
                      : 'p-paginator-prev p-paginator-element p-link me-2'
                  }
                ...
                >
                  <svg
                    ...
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                    className="p-icon p-paginator-icon mx-auto"
                    aria-hidden="true"
                  >
                    <path
                      fillRule="evenodd"
                ...
                <button
                  ...
                  className={
                    addClass
                      ? 'disabled p-paginator-prev p-paginator-element p-link mx-2'
                      : 'p-paginator-prev p-paginator-element p-link mx-2'
                  }
                  onClick={(e) => Pagination(page - 1)}
                >
                  <svg
                    width="14"
                    height="14"
                ...
                <span className="p-paginator-pages">
                  <button type="button" className="p-link-page mx-2">
                    {page}
                  </button>
                </span>
                <button
                  type="button"
                  className={
                    other.total <= other.cur
                      ? 'disabled p-paginator-next p-paginator-element p-link mx-2'
                      : 'p-paginator-next p-paginator-element p-link mx-2'
                  }
                  onClick={(e) => Pagination(page + 1)}
                >
                  <svg
                ...
                <button
                  ...
                  className={
                    other.total <= other.cur
                      ? 'disabled p-paginator-next p-paginator-element p-link ms-2'
                      : 'p-paginator-next p-paginator-element p-link ms-2'
                  }
                  onClick={(e) => Pagination(other.total)}
                >
                  <svg
                    width="14"
                    height="14"
              ...
              <div className="sm:mb-0 mb-3">
                <label className="pe-2 ps-auto">Jobs per page: </label>
                <select
                  className="w-16 rounded-lg border-[1.5px] border-stroke p-2 text-dark bg-white"
                  onChange={setLimit}
                >
