...
const View_team_content = () => {
  ...
  const fetchData = async () => {
    try {
      let data = await axios.get(
        BACKEND_COMMAN_URL + '/api/view_team_content',
        headers
      );
      if (data.status === 200) {
        setLoading(true);
  ...
  const Delete = async (id) => {
    let data = await axios.delete(
      `${BACKEND_COMMAN_URL}/api/deleteTeamContent/${id}`,
      headers
    );
    if (data.status === 200) {
      fetchData();
                      ...
                      <tr key={v._id}>
                        ...
                        <td className="border-b border-[#eee] py-4 px-3 dark:border-strokedark">
                          <div className="flex items-center">
                            <Link to={'/Update_team_content?id=' + v._id}>
                              <svg
...
export default View_team_content;
