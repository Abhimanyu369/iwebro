...
const Update_benefit = () => {
  ...
  const fetchData = async (e) => {
    ...
    let Fdata = await axios.put(BACKEND_COMMAN_URL + '/api/editBenefit/' + id, formData, headers);
    if (Fdata.status === 200) {
  ...
  const fetchData2 = async () => {
    const data = await axios.get(`${BACKEND_COMMAN_URL}/api/singleBenefit/${id}`, headers);
    if (data.status === 200) {
