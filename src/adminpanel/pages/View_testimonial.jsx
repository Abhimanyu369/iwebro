...
const View_testimonial = () => {
  ...
  const fetchData = async () => {
    try {
      let data = await axios.get(
        BACKEND_COMMAN_URL + '/api/view_testimonial',
        headers
      );
      if (data.status === 200) {
        setLoading(true);
  ...
  const Delete = async (id) => {
    let data = await axios.delete(
      `${BACKEND_COMMAN_URL}/api/deleteTestimonial/${id}`,
      headers
    );
    if (data.status === 200) {
      fetchData();
                      ...
                      <tr key={v._id}>
                        ...
                        <td className="border-b border-[#eee] py-4 px-3 dark:border-strokedark">
                          <div className="flex items-center">
                            <Link to={'/Update_testimonial?id=' + v._id}>
                              <svg
...
export default View_testimonial;
